/*
 * test_ur_account_descriptor_decoder.c
 *
 * Test URDecoder with crypto-account data
 * Reads .UR_fragments.txt files, decodes to crypto-account,
 * then extracts and verifies the output descriptor using
 * output_descriptor_from_cbor_account function.
 */
...
#define TEST_CASES_DIR "tests/test_cases/account"
...
static bool test_file(const char *filepath) {
  ...
  if (!fragments || fragment_count == 0) {
    fprintf(stderr, "Failed to read fragments from file: %s\n", filepath);
    return false;
  }
  ...
  if (ur_decoder_is_complete(decoder) && ur_decoder_is_success(decoder)) {
    ...
    if (result) {
      ...
      if (cbor_data && cbor_len > 0) {
        ...
        // Extract output descriptor from account CBOR
        char *actual_descriptor =
        ...
        if (actual_descriptor) {
          ...
          fprintf(stderr, "Failed to extract descriptor from account CBOR\n");
        }
      } else {
...
int main(int argc, char *argv[]) {
  return run_test_suite(argc, argv, "UR Account Descriptor Decoder Test",
