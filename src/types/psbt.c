...
psbt_data_t *psbt_from_cbor(const uint8_t *cbor_data, size_t len) {
    ...
    registry_item_t *item = registry_item_from_cbor(cbor_data, len, psbt_from_data_item);
    if (!item) return NULL;
