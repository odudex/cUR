...
registry_type_t ECKEY_TYPE = {.name = "crypto-eckey", .tag = CRYPTO_ECKEY_TAG};

// Create and destroy ECKey
ec_key_data_t *ec_key_new(const uint8_t *data, size_t data_len, int curve,
...
ec_key_data_t *ec_key_from_cbor(const uint8_t *cbor_data, size_t len) {
  ...
  registry_item_t *item =
      registry_item_from_cbor(cbor_data, len, ec_key_from_data_item);
  if (!item)
    return NULL;
