...
static bool encode_initial_byte(byte_buffer_t *buf, uint8_t major_type, uint64_t value) {
    if (value < 24) {
        ...
        uint8_t bytes[3] = {(major_type << 5) | 25, (uint8_t)(value >> 8), (uint8_t)value};
...
static bool encode_bytes(byte_buffer_t *buf, const uint8_t *data, size_t len) {
    if (!encode_initial_byte(buf, CBOR_MAJOR_BYTES, len)) return false;
    return byte_buffer_append(buf, data, len);
}

static bool encode_string(byte_buffer_t *buf, const char *str) {
    size_t len = strlen(str);
    if (!encode_initial_byte(buf, CBOR_MAJOR_STRING, len)) return false;
...
static bool encode_array(byte_buffer_t *buf, cbor_value_t *value) {
    ...
    if (!encode_initial_byte(buf, CBOR_MAJOR_ARRAY, count)) return false;

    for (size_t i = 0; i < count; i++) {
        cbor_value_t *item = cbor_value_get_array_item(value, i);
        if (!encode_value(buf, item)) return false;
    }
...
static bool encode_map(byte_buffer_t *buf, cbor_value_t *value) {
    ...
    if (!encode_initial_byte(buf, CBOR_MAJOR_MAP, count)) return false;

    if (value->type != CBOR_TYPE_MAP) return false;

    for (size_t i = 0; i < count; i++) {
        if (!encode_value(buf, value->value.map_val.keys[i])) return false;
        if (!encode_value(buf, value->value.map_val.values[i])) return false;
    }
...
static bool encode_tag(byte_buffer_t *buf, cbor_value_t *value) {
    ...
    if (!encode_initial_byte(buf, CBOR_MAJOR_TAG, tag)) return false;
...
// Encode CBOR value
bool urtypes_cbor_encoder_encode(urtypes_cbor_encoder_t *encoder, cbor_value_t *value) {
    if (!encoder || !value) return false;

    return encode_value(encoder->buffer, value);
}

// Get encoded data
uint8_t *urtypes_cbor_encoder_get_data(urtypes_cbor_encoder_t *encoder, size_t *out_len) {
    if (!encoder || !out_len) return NULL;
